...
separate_bign <- function(data, n_pattern = "N = ", create_bign_rows = TRUE) {
  ...
  # Extract Big N information from group1_level using pseudo-pattern system
  if ("group1_level" %in% names(data)) {
    unique_groups <- unique(data$group1_level)
    big_n_list <- list()

    for (group_text in unique_groups) {
      # Try to parse using pseudo-pattern system
      parsed_bign <- parse_bign_value_pseudo(group_text)

      if (!is.null(parsed_bign)) {
        big_n_list[[length(big_n_list) + 1]] <- data.frame(
          group1_level = group_text,
          group_label = if (!is.na(parsed_bign$label)) parsed_bign$label else "",
          n_value = parsed_bign$n_value,
          pattern_used = parsed_bign$pattern_used,
          stringsAsFactors = FALSE
        )
      }
    }

    if (length(big_n_list) > 0) {
      big_n_info <- do.call(rbind, big_n_list)
    } else {
      # Fallback to legacy pattern if pseudo-patterns don't match
      big_n_info <- data |>
        distinct(group1_level) |>
        filter(grepl("[Nn]\\s*[=:]?\\s*\\d+", group1_level)) |>
        mutate(
          group_label = stringr::str_extract(group1_level, ".*?(?=\\(?[Nn]\\s*[=:])"),
          n_value = stringr::str_extract(group1_level, "[Nn]\\s*[=:]?\\s*(\\d+)") |>
            stringr::str_extract("\\d+")
        ) |>
        mutate(
          group_label = stringr::str_trim(group_label),
          n_value = as.numeric(n_value),
          pattern_used = "legacy_fallback"
        ) |>
